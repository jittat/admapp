...
@user_login_required
def index(request, criterion_id):
    user = request.user
    # mock
    project_id = 1
    round_id = 2
    ...
    majors = project.major_set.select_related('faculty').order_by('faculty_id').all()

    if faculty:
        majors = [m for m in majors if m.faculty_id == faculty.id]

    return render(request,
                  'backoffice/criterion/index.html',
                  { 'project': project,
...
def create(request, project_id, round_id):
    ...
    return render(request,
                  'backoffice/criterion/create.html',
                  { 'project': project,
                    'admission_round': admission_round,
                    'faculty': faculty,
                    'majors': majors,

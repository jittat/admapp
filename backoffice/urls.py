...
urlpatterns = [
    ...
    re_path(r'^payment/update/(?P<payment_id>\d+)/$',
            payments.update, name='payment-update'),

    re_path(r'^applicants/([0-9a-zA-Z\d]\d+)/$',
            views.show, name='show-applicant'),
    re_path(r'^applicants/(\d+)/([0-9a-zA-Z]\d*)/$',
            views.show, name='show-applicant-in-project'),

    re_path(r'^new_password/([0-9a-zA-Z]\d+)/$',
            views.new_password, name='new-password'),
    re_path(r'^update/([0-9a-zA-Z]\d+)/$',
    ...
    re_path(r'^appinfo/$', projectoptions.show_project_application_info,
            name='show_project_application_info'),
    re_path(r'^options/$', projectoptions.show_project_options,
            name='show_project_options'),
    re_path(r'^options/(\d+)/(.+)/(\d+)/$', projectoptions.update_project_options,
            name='update_project_options'),
    
    re_path(r'^projects/(\d+)/(\d+)/$', projects.index, name='projects-index'),
    re_path(r'^projects/(\d+)/(\d+)/list/$',
            projects.list_applicants, name='projects-list'),
    re_path(r'^projects/(\d+)/(\d+)/majors/$',
            projects.list_major_details, name='projects-list-majors'),
    re_path(r'^projects/(\d+)/(\d+)/interview-descriptions/$', projects.list_major_interview_descriptions,
            name='projects-list-majors-interview-descriptions'),

    re_path(r'^projects/applicants/(?P<project_id>\d+)/(?P<round_id>\d+)/(?P<major_number>\d+)/(?P<rank>\d+)/$',
            projects.show_applicant, name='projects-show-applicant'),

    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/doc/(\d+)/(\d+)/(\d+)/$',
            projects.download_applicant_document,
            name='projects-download-app-document'),

    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/(\d+)/marks/(\d+)/$',
            projects.check_mark_toggle,
            name='projects-check-mark-toggle'),

    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/(\d+)/comments/$',
            projects.save_comment,
            name='projects-save-comment'),
    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/(\d+)/comment/(\d+)/delete/$',
            projects.delete_comment,
            name='projects-delete-comment'),

    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/(\d+)/criteria/(.+)/$',
            projects.set_criteria_result,
            name='projects-set-criteria-result'),
    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/(\d+)/interview/(.+)/$',
            projects.set_call_for_interview,
            name='projects-set-call-for-interview'),
    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/(\d+)/acceptance/(.+)/$',
            projects.set_acceptance,
            name='projects-set-acceptance'),

    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/sheet$',
            reports.download_applicants_sheet,
            name='projects-download-app-sheet'),

    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/sheet/interview$',
            reports.download_applicants_sheet,
            {'only_interview': True},
            name='projects-download-app-sheet-only-interview'),

    re_path(r'^projects/applicants/(\d+)/(\d+)/(\d+)/interview-sheet$',
    ...
    re_path(r'^projects/scores/(?P<project_id>\d+)/(?P<round_id>\d+)/(?P<major_number>\d+)/$',
            projects.show_scores, name='projects-show-scores'),
    re_path(r'^projects/scores/(?P<project_id>\d+)/(?P<round_id>\d+)/(?P<major_number>\d+)/interview_score/$',
            projects.update_interview_call_score, name='projects-interview-call-score-update'),

    re_path(r'^projects/calls/(?P<project_id>\d+)/(?P<round_id>\d+)/(?P<major_number>\d+)/$',
            projects.list_applicants_for_acceptance_calls, name='projects-list-applicants-for-acceptance-calls'),
    re_path(r'^projects/calls/(?P<project_id>\d+)/(?P<round_id>\d+)/(?P<major_number>\d+)/update/$',

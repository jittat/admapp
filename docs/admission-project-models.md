...
| `is_cupt_export_zero_score_fields` | `False` | "ส่งข้อมูลทปอ.โดยไม่ส่งเกณฑ์ขั้นต่ำและคะแนน" — half way between the flag above being `True` and `False`: rows stay **per-criteria** (not combined, `add_limit` normal — except for portfolio projects, where it is always `0`), but min scores and scoring weights all export as `0`. The portfolio/interview split still carries its values. |
